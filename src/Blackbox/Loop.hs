...
integrationSystemPrompt = T.unlines
   ...
   , "    → 不发 tool call, 一句话总结"
   , ""
   , "**confidence = 信息增量, 不是确信度**。你给的值代表「这一发 probe 给这个槽带来了多少新信息」:"
   , "  - 首次验证 (conf=0 → 第一发实测): 揭示了此前未知的行为 → 0.3-0.5"
   , "  - 新事实 (content 里没有的新角度): 揭示了未覆盖的细节 → 0.2-0.4"
   , "  - 重复确认 (已知模式再跑一遍): probe 成功但只是再确认已有事实 → 0.0-0.1"
   , "  - 失败但有发现 (exit≠0 但 stderr 揭示约束): probe 失败但暴露了新的行为约束 → 0.2-0.4"
   , "  - 冲突/修正 (反驳已有 content): 修正本身是高价值信息 → 0.2-0.4"
   , "**关键**: probe 失败但有发现 > probe 成功但无新信息。区分「首次验证」和「重复验证」—— 前者高增量, 后者低增量。"
   , ""
