...
当前没有真实 LLM 时，Codex 临时代替 LLM 做 archetype decision、binding
generation 和 result evaluation 的机械执行清单在
`docs/pb/codex-llm-runbook.md`。新窗口如果不知道第四类任务类型或
binding-driven flow 怎么跑，先读这个文件，不要从聊天记录猜。
...
- `src/Blackbox/DTC/Requirements.hs`: archetype 参数需求入口，当前支持 `WatcherCli` / `HttpClientCli` / `StructuredSubcommandCli` / `TabularRenderCli`。
...
- Linux amd64 版 `hsbb` 已在 PB task image 内编译成功，runner 可把该 host
  缓存注入 PB task container；路径语义见 `docs/pb/README.md`。
...
- `dtc requirements TabularRenderCli` 会输出表格/分隔符渲染 CLI 的 binding
  字段，供 csview/xsv 等任务抽取 stdin/file 输入、CSV/TSV 或自定义分隔符、
  样式输出、文件错误路径，以及可选的 no-header、序号列、布局控制、宽字符
  和畸形输入错误路径。新增字段必须保持表格渲染 CLI 共性，不允许把 csview-only
  长尾硬塞进 archetype。
...
- PB 真实 reference 环境验证：把 Linux `hsbb` 注入 PB task 容器，与
  ...
  `14/14 Pass`，`csview` 为 `11/11 Pass`。精简 `results.jsonl` 已另存到
  `docs/pb/results/`；host runner 输出只作取证缓存，不作为执行互通机制。
- 这些 Pass 只表示当前 DTC plan 中的 expectation 成立，不表示项目完整正常
  ...
  响应面已验证；`csview 11/11` 代表表格渲染 CLI 的 stdin/file、CSV/TSV、
  delimiter/style、missing-file、version、no-header、序号列、布局控制、宽字符、
  畸形输入错误等共性面已验证。未覆盖行为仍需继续从 source/grader 抽取。
- PB 同容器 runner 已产品化为 `scripts/pb-dtc-runner.sh`。它会推断 task image、
  复用或重建 host 侧 Linux `hsbb` 缓存，创建一次性 task container，
...
- `ariga__atlas.6d81150` 已开始首探：image
  ...
  `completion bash`，均 exit 0。这些首探输出只是当轮 runner host out
  取证缓存，不作为长期路径依赖。
...
- atlas binding-driven 同容器 DTC 最初在源码 corpus 缺失时扩出 provisional
  ...
  错误路径、`--config/--env/--var` 配置驱动 schema inspect。当前可复核的
  精简结果已保存到 `docs/pb/results/atlas-20260701-083041-results.jsonl`；
  更早的 provisional/source-audited host out 只作为历史调试缓存，不应作为
  新窗口复跑依据。
...
- `wfxr__csview.8ac4de0` 已作为 `TabularRenderCli` seed 接入：upstream source
  锁定到 `8ac4de0ae4540461fd9e33c13e4b47ba21967995`，PB metadata
  `tests.json` 已落到 `corpus/probe-plan-seeds/csview/grader/tests.json`，
  binding 在 `docs/pb/bindings/wfxr__csview.8ac4de0.json`。当前同容器
  source-grounded flow 11/11 Pass：help、stdin CSV、file + delimiter/style、
  stdin TSV + markdown style、missing file、version，以及 generic optional
  no-header、sequence column、layout controls、wide characters、malformed input
  error。后续扩表格类任务时继续只补共性面；太刻意或 csview-only 的流程必须
  留在项目 binding/后续专用 archetype 外，不要污染 `TabularRenderCli`。
- 当前四个 seed 的精简 DTC result evidence 已落到 `docs/pb/results/`：
  entr、bat、atlas、csview 各一个 `results.jsonl`。后续新增 PB seed 时，
  若结果文件体积小，也应把关键 run 的 `results.jsonl` 复制到该目录。
...
3. Codex 替代 LLM 时按 `docs/pb/codex-llm-runbook.md` 的 12 步执行；
   第四类 `TabularRenderCli` 也必须走 `requirements -> binding ->
   validate-binding -> plan-binding -> 同容器 run-binding`，不要手写项目命令。
4. 不要恢复旧 LLM loop。先保持 Haskell DTC 为执行和验证核心；DeepSeek 只能消费 `system-prepare` 的机械读取包做系统层决策/评估/oracle 提案。
5. 类 entr/bat/atlas/csview 任务不要复制旧 seed 的具体 step；先跑对应
   `dtc requirements <Archetype>`，再让决策节点从 source/upstream
   tests/grader/help 中抽取 binding，并用 reusable flow builder 生成流程。
6. 如果新项目暴露共性缺口，再补 archetype 或 runtime：readiness gate、
   structured command、artifact index、HTTP request evidence 等都属于被任务牵引的
   支撑工作，不是单独优先项。
7. bat 后续如果被选中继续扩，不要堆随机 grader case；优先补 request artifact
   index、URL shorthand、proxy/TLS/large or streaming response 这些主流行为面。
8. atlas 若回头继续扩，应在 source-audited 13-step 起点上继续扩
