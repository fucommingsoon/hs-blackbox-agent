...
3. `docs/pb/codex-llm-runbook.md`: 当前没有真实 LLM 时，Codex 临时代替 LLM
   做 archetype decision、binding generation、result evaluation 的机械清单。
4. `docs/pb/results/README.md`: 已验证 seed 的精简 DTC result evidence。
5. `docs/pb/tasks.md`: 当前本地 ProgramBench 201 个 task 清单。
6. `AGENTS.md`: 本地构建、运行和开发约束。
7. `FLOW.md`: DTC 构建流程和 Agent 执行流程。
8. `TODO.md`: 当前待办。
...
Codex 临时代替 LLM 的 binding-driven 执行清单见
